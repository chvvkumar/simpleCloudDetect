...
class CloudDataset(Dataset):
    def __init__(self, image_paths, labels, transform=None, cache_size_gb=0):
            ...
            for i, path in enumerate(image_paths):
                try:
                    with Image.open(path) as img:
                        ...
                        est_size = img.size[0] * img.size[1] * 3
    ...
    def __getitem__(self, idx):
        ...
        while attempt < max_attempts:
            ...
            try:
                ...
                # print(f"⚠️ Corrupt: {path} - Trying next...")
...
def train_model(data_dir, output_model='model.onnx', output_labels='labels.txt', 
    ...
    # Datasets
    if cache_ram_gb > 0:
        print(f"🧠 Initializing Datasets (Max RAM: {cache_ram_gb:.1f} GB)...")
    
    train_dataset = CloudDataset(train_paths, train_labels, transform=train_transforms, cache_size_gb=train_cache_gb)
    val_dataset = CloudDataset(val_paths, val_labels, transform=val_transforms, cache_size_gb=val_cache_gb)

    loader_kwargs = {
        'batch_size': batch_size,
        'num_workers': num_workers,
        'pin_memory': True,
        # Only use persistent_workers if we actually have workers
    ...
    for epoch in range(epochs):
        ...
        print(f"   ⏱️  Timings: Data: {avg_data_time*1000:.1f}ms | GPU: {avg_compute_time*1000:.1f}ms")
