...
# This module is an integration smoke test that drives the alpyca *client*
# library (alpaca.safetymonitor.SafetyMonitor) against a live ASCOM Alpaca
# SafetyMonitor device over HTTP (DEVICE_ADDRESS below). It requires both the
# external alpyca client package to be installed and a reachable running
# Alpaca server/device. Neither is available in unit-test CI, so skip the
# whole module rather than crash collection on the import or hang on sockets.
pytest.importorskip(
    "alpaca.safetymonitor",
    reason="requires the external alpyca client library and a running Alpaca SafetyMonitor device",
)
pytest.skip(
    "requires a running Alpaca SafetyMonitor server/device at the configured DEVICE_ADDRESS",
